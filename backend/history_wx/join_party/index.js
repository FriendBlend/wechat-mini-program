...
exports.main = async (event, context) => {
  ...
  try {
    ...
    // 检查座位序号是否有效
    if (seat_index < 0 || seat_index >= party.member_num) {
      await transaction.rollback()
      return {
        success: false,
    ...
    // 检查座位是否已被占用
    if (party.seat_list && party.seat_list[seat_index]) {
      await transaction.rollback()
      return {
        success: false,
    ...
    // 将用户ID添加到seat_list的指定位置
    const updatedSeatList = party.seat_list ? [...party.seat_list] : []
    updatedSeatList[seat_index] = user_id
